...
@interface JBScrollContentView()<PhotoDelegate>
...
@end
@implementation JBScrollContentView
- (UIPageControl*)pageControl
{
    if (!_pageControl)
    {
        _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(0,JBScreen_Height-90,JBScreen_Width, 30)];
...
- (void)loadImageUrls:(NSArray *)images defaultIndex:(NSInteger)index currentSelectIndex:(selectBlock)indexBlock
{
    ...
    for (int i =0; i<images.count; i++)
    {
        ...
        JBImageView *imageView = [[JBImageView alloc]initWithFrame:CGRectMake(X,0,viewWidth,viewHeight)];
